...
import { MealPlanStatus, Role } from '../../generated/prisma/client';
...
describe('MealPlansService (integração)', () => {
  ...
  beforeAll(async () => {
    ...
    tenantA = await prisma.tenant.create({
      data: { name: 'Tenant MealPlan A', slug: `mp-a-${runId}`, email: 'a@teste.com', phone: '11111111' },
    });
    tenantB = await prisma.tenant.create({
      data: { name: 'Tenant MealPlan B', slug: `mp-b-${runId}`, email: 'b@teste.com', phone: '22222222' },
    });

    nutritionistA = await prisma.user.create({
      data: { name: 'Nutri MealPlan A', email: `nutri-mp-a-${runId}@teste.com`, passwordHash: 'x' },
    });
    ...
    receptionA = await prisma.user.create({
      data: { name: 'Recepção MealPlan A', email: `reception-mp-a-${runId}@teste.com`, passwordHash: 'x' },
    });
    await prisma.userClinic.create({
      data: { userId: receptionA.id, tenantId: tenantA.id, role: Role.RECEPTION },
    });

    patientA = await prisma.patient.create({ data: { tenantId: tenantA.id, fullName: 'Paciente MealPlan A' } });
    patientB = await prisma.patient.create({ data: { tenantId: tenantB.id, fullName: 'Paciente MealPlan B' } });
  }, 20000);
  ...
  afterAll(async () => {
    ...
    await prisma.userClinic.deleteMany({ where: { userId: { in: [nutritionistA.id, receptionA.id] } } });
    await prisma.user.deleteMany({ where: { id: { in: [nutritionistA.id, receptionA.id] } } });
    await prisma.tenant.delete({ where: { id: tenantA.id } });
    await prisma.tenant.delete({ where: { id: tenantB.id } });
    await prisma.$disconnect();
  });

  it('cria um plano com refeições, itens e substituições aninhados', async () => {
          ...
          items: [
            {
              ...
              quantity: 2,
              unit: 'fatias',
              substitutions: [{ description: 'Tapioca', quantity: 1, unit: 'unidade' }],
            },
          ],
        },
      ],
    ...
    expect(created.meals[0].items[0].substitutions).toHaveLength(1);
    expect(created.meals[0].items[0].substitutions[0].description).toBe('Tapioca');
  });
  ...
  it('isola planos por tenant — paciente de outro tenant não é encontrado', async () => {
    await expect(
      service.create(tenantA.id, nutritionistA.id, Role.NUTRITIONIST, patientB.id, {
  ...
  it('update com meals substitui a árvore inteira (nunca mescla)', async () => {
    const created = await service.create(tenantA.id, nutritionistA.id, Role.NUTRITIONIST, patientA.id, {
    ...
    const updated = await service.update(tenantA.id, nutritionistA.id, created.id, {
      meals: [{ name: 'Jantar', items: [{ description: 'Sopa' }] }],
    });

    expect(updated.meals).toHaveLength(1);
    expect(updated.meals[0].name).toBe('Jantar');
    expect(updated.meals.find((m) => m.name === 'Almoço')).toBeUndefined();
  });

  it('activate marca o plano anterior ACTIVE como REPLACED, nunca apaga', async () => {
    const first = await service.create(tenantA.id, nutritionistA.id, Role.NUTRITIONIST, patientA.id, {
      title: 'Plano v1 ativo',
      effectiveFrom: '2026-08-01',
    });
    await service.activate(tenantA.id, nutritionistA.id, first.id);

    const second = await service.create(tenantA.id, nutritionistA.id, Role.NUTRITIONIST, patientA.id, {
      title: 'Plano v2 substituto',
      effectiveFrom: '2026-08-15',
    });
    const activatedSecond = await service.activate(tenantA.id, nutritionistA.id, second.id);
  ...
  it('só permite ativar um plano em DRAFT', async () => {
    ...
    await expect(service.activate(tenantA.id, nutritionistA.id, created.id)).rejects.toBeInstanceOf(
      BadRequestException,
    );
  });

  it('duplicate cria uma cópia totalmente independente (sem parentMealPlanId)', async () => {
    const original = await service.create(tenantA.id, nutritionistA.id, Role.NUTRITIONIST, patientA.id, {
      title: 'Plano original para duplicar',
      effectiveFrom: '2026-08-01',
      meals: [{ name: 'Lanche', items: [{ description: 'Iogurte' }] }],
    });

    const copy = await service.duplicate(tenantA.id, nutritionistA.id, original.id);

    expect(copy.id).not.toBe(original.id);
    expect(copy.parentMealPlanId).toBeNull();
    expect(copy.version).toBe(1);
    expect(copy.status).toBe(MealPlanStatus.DRAFT);
    expect(copy.meals[0].name).toBe('Lanche');

    // Editar a cópia não deve afetar o original — árvores independentes.
    await service.update(tenantA.id, nutritionistA.id, copy.id, {
      meals: [{ name: 'Lanche editado' }],
    });
    const originalAfter = await service.getById(tenantA.id, original.id);
    expect(originalAfter.meals[0].name).toBe('Lanche');
  });

  it('new-version cria uma versão encadeada e mantém o plano original intocado', async () => {
    const original = await service.create(tenantA.id, nutritionistA.id, Role.NUTRITIONIST, patientA.id, {
    ...
    const nextVersion = await service.createNewVersion(tenantA.id, nutritionistA.id, original.id);
  ...
  it('archive é soft delete — some da listagem mas o registro permanece', async () => {
    ...
    const archived = await service.archive(tenantA.id, nutritionistA.id, created.id);
  ...
  it('share não expõe internalNotes — só isSharedWithPatient e patientVisibleNotes mudam', async () => {
    ...
    const shared = await service.share(tenantA.id, nutritionistA.id, created.id, {
    ...
    expect(shared.internalNotes).toBe('Nota interna sensível — nunca para o paciente');
