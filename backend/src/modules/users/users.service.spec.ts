...
describe('UsersService.changeOwnPassword (integração)', () => {
  ...
  it('troca a senha quando a atual está correta, e a nova senha passa a autenticar', async () => {
    ...
    const matchesNew = await bcrypt.compare('NovaSenha456', updated.passwordHash);
    const matchesOld = await bcrypt.compare(INITIAL_PASSWORD, updated.passwordHash);
  ...
  it('grava a troca no AuditLog, sem expor a senha', async () => {
    ...
    expect((entry?.metadata as { changeType?: string } | null)?.changeType).toBe(
